...
sub new
{
	...
	&::printEvent("","Created new player object " . $self->getInfoString(),2);
	return $self;
}
...
sub getAddress
{
	...
	elsif ($::g_stdin == 0 && $self->{is_bot} == 0 && ( ($self->{userid} > 0) || (($self->{userid} == 0) && ($::g_servers{$self->{server}}->{play_game} == CS2())) ))
	{
		...
		&::printEvent("RCON","rcon_getaddress",2);
		...
		if (defined $result->{Address} && $result->{Address} ne "") {
			...
			&::printEvent("RCON", "Got Address $self->{address} for Player $self->{name}", 2,1);
		}
	}
