...
sub DoGeoIP
{
	...
	if ($useGeoIPBinary == 0)
	{
		...
		{
			&printEvent("ERROR", "GeoIP method set to database but geoLiteCity tables are empty.", 1,1);
		}
	}
	elsif ($useGeoIPBinary == 1 && -r $geoipfile)
	{
		...
		{
			&printEvent("ERROR", "GeoIP method set to binary file lookup but $geoipfile errored while opening.", 1,1);
			$gi = undef;
		}
	}
	else
	{
		&printEvent("ERROR", "GeoIP method set to binary file lookup but $geoipfile NOT FOUND", 1,1);
	}
